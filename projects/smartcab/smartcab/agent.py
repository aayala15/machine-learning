...
class LearningAgent(Agent):
    ...
    def __init__(self, env):
        ...
        self.goal = np.zeros(50)
        self.right = np.zeros(50)
        self.wrong = np.zeros(50)
    ...
    def reset(self, destination=None):
        ...
        self.reward = 0.0
        self.n = self.n + 1
        self.epsilon = self.epsilon * np.exp(-1/20)
    ...
    def update(self, t):
        ...
        if self.n >= 50:
            self.time[self.n-50] = self.time[self.n-50] + 1
            if reward < 0:
                self.wrong[self.n-50] = self.wrong[self.n-50] + 1
            elif reward > 0:
                self.right[self.n-50] = self.right[self.n-50] + 1
            if reward >= 10 and deadline > -1:
                self.goal[self.n-50] = 1
